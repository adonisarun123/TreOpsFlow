...
### Recently Fixed Bugs (Apr 2026)

| # | Bug | Priority | Fix | Status |
|---|-----|----------|-----|--------|
| 1 | Sales can't edit Stage 1 after creation | P0 | Changed conditional to show Stage1Form for owner at Stage 1 regardless of rejection; hid HandoverActions from owner | ✅ Fixed |
| 2 | Emails not reaching correct recipients | P0 | Root cause was Bug 8 (email system broken). Recipients in code were already correct | ✅ Not a bug |
| 3 | Admin can't edit user details | P2 | Added `updateUser()` server action, `EditUserForm` dialog, status/actions columns in team page | ✅ Fixed |
| 4 | Admin can't delete events with reason | P2 | Updated delete API to accept reason, added `programDeletedEmail` template, `DeleteProgramModal`, trash icons on Kanban cards + programs table (Admin only) | ✅ Fixed |
| 5 | Kanban cards lack key info | P1 | Added Program Type badge, Location (MapPin), Pax range (Users) to kanban cards | ✅ Fixed |
| 6 | Reports missing key metrics | P2 | Added `getAverageZFDScore()`, `getPendingFinanceCount()`, `getNeedsAttention()` server actions; new KPI cards + "Needs Attention" section in reports | ✅ Fixed |
| 7 | Non-admin users see admin features | P1 | RBAC was already correct in code; Bug 1 fix completed the gap | ✅ Not a bug |
| 8 | Email links broken in production | P0 | Changed static `APP_URL` to runtime `getAppUrl()` function in email templates | ✅ Fixed |
| 9 | Double rejection on programs | P0 | Changed `router.refresh()` to `router.push('/dashboard')` after rejection actions | ✅ Fixed |
| 10 | Upload 403 on custom subdomain | P0 | Added `allowedOrigins` to server actions config for CSRF origin whitelisting | ✅ Fixed |
...
| 15 | Fix email template APP_URL (runtime getAppUrl) | ✅ Done |
| 16 | Fix Sales can't edit Stage 1 form | ✅ Done |
| 17 | Fix double-rejection (router.push instead of refresh) | ✅ Done |
| 18 | Add key info to Kanban cards (type, location, pax) | ✅ Done |
| 19 | Admin edit user (updateUser action + EditUserForm) | ✅ Done |
| 20 | Admin delete with reason + email notification | ✅ Done |
| 21 | Reports: ZFD score, pending finance, needs attention KPIs | ✅ Done |
| 22 | Fix subdomain upload 403 (server action allowedOrigins) | ✅ Done |
...
*This analysis was generated by reviewing every file in the TreOpsFlow codebase. Last updated: 2026-04-01.*
